...
<html>
	...
	<body>
		...
		<ec:table items="list"
			action="${pageContext.request.contextPath}/DeviceStatus-getAllUtil.action"
			imagePath="${pageContext.request.contextPath}/common/img/table/*.gif"
			title="设备状态列表" width="200%" rowsDisplayed="5" var="tmp"><ec:exportXls fileName="DeviceStatus.xls" tooltip="Excel" /><ec:row>
				...
				<ec:column sortable="false" filterable="false" viewsDenied="xls"
					alias="id2" title="具体操作">
						<c:if test="${type == 'admin'}">
							<a target="Conframe"
								href="${pageContext.request.contextPath}/DeviceStatus-selectUtil.action?util.id=${tmp.id }">修改
							</a>
							<a target="Conframe"
										href="${pageContext.request.contextPath}/DeviceStatus-deleteUtil.action?util.id=${tmp.id }">删除
							</a>
						</c:if>
								<c:if test="${type != 'admin'}">
									<a target="Conframe"
										href="${pageContext.request.contextPath}/DeviceStatus-selectUtil.action?util.id=${tmp.id }">查看
							</a>
						</c:if>
				</ec:column>
				</ec:row>
